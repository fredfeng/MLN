 ...
 */
public class MaxSatGenerator implements HasTimeout {
    ...
    private List<FormattedConstraint> buildMaxSatQuestion(
        ...
        // Mcsls assumes that weight 1 is used only by absCost.
        for (FormattedConstraint c : bias) {
            c.weight(Math.max(2, c.weight()));
        }
        ...
        ProfileUtil.start("MAIN", "bias-cost");
        {
            ...
            for (ParameterValue pv : maxAbstraction) if (pv.value < maxK) {
...
foo:
    ...
    @Override public void setTimeout(Timeout timeout) {
